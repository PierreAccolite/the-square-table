...
HAND_MODEL=os.path.join(BASE,'hand_landmarker.task'); FACE_MODEL=os.path.join(BASE,'face_landmarker.task')
HAND_URL='https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task'; FACE_URL='https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/1/face_landmarker.task'
app=Flask(__name__); cam_lock=threading.Lock(); frame_lock=threading.Lock(); state_lock=threading.Lock()
camera=None; camera_index=None; latest_jpeg=None; frame_seq=0; running=True; hand_tracker=face_tracker=None; ts=0; fps=0.; fps_samples=deque(maxlen=30)
...
def process(frame):
    ...
    if fr and fr.face_landmarks:
        ...
        def ear(ids):p=[fl[i] for i in ids];return(D(p[1],p[5])+D(p[2],p[4]))/(2*max(D(p[0],p[3]),.0001))
    ...
    if grab:
        if not ball['held']:ball['vx']=ball['vy']=0.;ball['lastx']=grab['x'];ball['lasty']=grab['y'];ball['lastt']=now
        dt=max(now-ball['lastt'],.016);ball['vx']=(grab['x']-ball['lastx'])/dt*.7;ball['vy']=(grab['y']-ball['lasty'])/dt*.7;ball.update(held=True,holder=grab['label'],x=grab['x'],y=grab['y'],lastx=grab['x'],lasty=grab['y'],lastt=now)
    elif ball['held']:
